/**
 * @overview HTML templates of <i>ccm</i>-based web component for a rally board game.
 ...
 * @returns {string}
 */
export function chooseRacetrack(racetracks, path, size) {
  return `
    <section id="choose_racetrack">
      <h1 data-lang="choose_racetrack"></h1>
      <div>
        ${racetracks.map(({name, board}, i) => `
          <article>
            <h2>${name}</h2>
            <div class="board" style="grid-template-columns: repeat(${board[0].length}, ${size}px); grid-template-rows: repeat(${board.length}, ${size}px);"
                 onclick=%onSelectedRacetrack%>
...
export function board(board, objects, path, size) {
  return html`
    <main>
      ...
      <div>
        <article class="board"
                 style="padding: ${size}px; grid-template-columns: repeat( ${board[0].length}, ${size}px ); grid-template-rows: repeat( ${board.length}, ${size}px );">
          ${board.map(row => row.map(field_id => field(field_id, path)))}
        </article>
...
function field(id, path) {
  return id === '_____' ? html`
    <div class="field"></div>` : html`
    <div class="field" style="background-image: url( ${path + 'images/fields/' + id + '.jpg'} )"></div>`;
}
...
function object(obj, path) {
  return html`<img src="${path + 'images/objects/' + obj.type + (obj.nr || '') + '.png'}" class="obj"
                   width="${obj.width}" height="${obj.height}"
                   style="transform: rotate( ${(obj.direction || 0) * 90}deg )"></img>`;
}
